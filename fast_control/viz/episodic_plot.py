...
def episodic_plot_eval_for_controller(x_0, epochs, path, gp_names):
    ...
    for gp_data, gp_name in zip(gp_zs, gp_names):
        ...
        fig.colorbar(sm, ticks=np.arange(1.0, epochs + 1))
        ax.legend()
        ...
        fig.tight_layout()
        fig.savefig(
            f"plots/acrobat/{gp_name} controller over time -time{int(time.time())}.png"
        )
        plt.show()
        plt.close()
...
def episodic_plot_cum_diff_from_oracle_for_controller(
    x_0, epochs, path, gp_names, c_cdot=0
):
    ...
    for i, gp_name in enumerate(gp_names):
        ...
        fig.tight_layout()
        fig.savefig(
            f"plots/acrobat/cumdiff_from_oracle_{gp_name}_time{int(time.time())}.png"
        )
        plt.show()
        plt.close()


def episodic_plot_predicted_vs_true_eval(x_0, epochs, T, num_steps):
    ...
    for key in data.keys():
        ...
        fig.figsize = (9, 6)
        fig.tight_layout()
        fig.savefig(f"plots/acrobat/{key} predicted-true z-time{int(time.time())}.png")
        plt.show()
        plt.close()


def episodic_plot_cum_predicted_vs_true_eval(x_0, epochs, T, num_steps):
    ...
    for key in data:
        ...
        fig.tight_layout()
        fig.savefig(
            f"plots/acrobat/{key} cum error in df pred-time{int(time.time())}.png"
        )
        plt.show()
        plt.close()


def episodic_plot_u_for_controller(epochs, path, gp_names):
    ...
    for i, gp_name in enumerate(gp_names):
        ...
        fig.tight_layout()
        fig.savefig(
            f"plots/acrobat/u for {gp_name} controller over time -time{int(time.time())}.png"
        )
        plt.show()
        plt.close()
