...
def episodic_plot_func_for_controller(x_0, epochs, path, gp_names):
    ...
    for gp_data, gp_name in zip(gp_zs, gp_names):
        ...
        fig.tight_layout()
        fig.savefig(
            f"dip_plots/{gp_name} controller over time -time{int(time.time())}.png"
        )
        plt.show()
        plt.close()
...
def episodic_plot_predicted_vs_true_func(x_0, epochs, T, num_steps):
    ...
    for key in data.keys():
        ...
        fig.figsize = (9, 6)
        fig.tight_layout()
        fig.savefig(f"dip_plots/{key} predicted-true z-time{int(time.time())}.png")
        plt.show()
        plt.close()
...
def episodic_plot_cum_predicted_vs_true_func(x_0, epochs, T, num_steps):
    ...
    for key in data.keys():
        ...
        for epoch in range(2, epochs):
            ax.plot(
                ts, csum[:, epoch], label=epoch, markersize=c, alpha=0.4, c=cmap(epoch)
            )
        ...
        fig.figsize = (9, 6)
        fig.tight_layout()
        fig.savefig(f"dip_plots/{key} cum error in df pred-time{int(time.time())}.png")
        plt.show()
        plt.close()
